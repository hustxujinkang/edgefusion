...
| 类别 | 字段 | 必要性 | 说明 |
|------|------|--------|------|
| 读 | `power` | 必需 | 总表功率，必须确认正负方向 |
| 读 | `status` | 建议 | 在线/离线/故障状态，必要时配 `status_map` |
...
|------|------|--------|------|
| 读 | `power` | 必需 | 实时发电功率 |
| 读 | `status` | 建议 | 运行状态，必要时配 `status_map` |
| 写 | `power_limit` | 建议 | 如果要参与闭环控制 |
...
- `power` 能稳定读到
- `status` 能归一
- 如有控制需求，`power_limit` 至少一次下发成功

### 1.3 储能 `energy_storage`
...
### 1.4 充电桩 `charging_station` / `charging_connector`
